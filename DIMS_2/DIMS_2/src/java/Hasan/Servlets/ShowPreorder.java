 ...
 * @author Hasan Murad
 */
@WebServlet(name = "ShowPreorder", urlPatterns = {"/ShowPreorder.do"})
public class ShowPreorder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       // Vector<orderproduct> exist=db.getorderlist(e.employee_id);
        Vector<Retailer> exist2=db.getOrderRetailerlist(e.employee_id);
        if(exist2!=null)
        {
            session.setAttribute("retailerlist",exist2);
            
         //   session.setAttribute("orderproduct",exist);
            RequestDispatcher rd = request.getRequestDispatcher("OrderRetailer.jsp");
